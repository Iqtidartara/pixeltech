    ...
    >
      ...
      <div className="w-full max-w-full flex flex-col mt-5">
          ...
          {features.map((feature, index) => (
            <div key={index} className="flex gap-4 items-center p-2">
              <Image
                ...
                height={85}
              />
              <p className="text-base md:text-lg font-medium leading-[150%]">
                {feature}
              </p>
            </div>
          ))}
        ...
        <div className="flex flex-col gap-4 mt-6">
          <button
            className={`px-4 py-2 rounded-[37.48px] hover:bg-[#FAFAFA] hover:text-[#000] ${buttonStyles[type]}`}
          >
            {type === "pro" ? "Upgrade Now" : "Get Started"}
          </button>
          <button
            data-cal-namespace=""
            data-cal-link="faisal-imtiaz-o7ho3v/15min"
            data-cal-config='{"layout":"month_view"}'
            className={`px-4 py-2 rounded-[37.48px] hover:bg-[#fafafada] bg-[#FAFAFA] text-[#323232] text-[16px] font-[700] `}
          >
            Book A Call
          </button>
        </div>
